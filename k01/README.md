...
## ソースコードの説明
l.65 リニアサーチの実装。  
l.68 左から順にIDが一致するか探索。  
l.69 一致すればindexを返す。  
l.73 不一致なら-1を返す。  
l.76 バイナリサーチの実装。  
l.80 左端indexの値が右端indexの値を超えるまで繰り返す。  
l.81 配列の中央のindexを求める。  
l.82 IDが一致すればindexを返す。  
l.84 中央indexのIDが入力IDよりも小さければ、左端を中央の1つ右に移し繰り返す。  
l.86 中央indexのIDが入力IDよりも大きければ、右端を中央の1つ左に移し繰り返す。  
l.90 不一致なら-1を返す。  
